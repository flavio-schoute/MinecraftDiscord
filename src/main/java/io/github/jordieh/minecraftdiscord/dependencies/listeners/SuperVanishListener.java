...
public class SuperVanishListener implements Listener {
    ...
    public SuperVanishListener(Dependency dependency) {
        this.pair = new Pair<>(dependency.name, dependency.icon);
    }

    @EventHandler(ignoreCancelled = true, priority = EventPriority.MONITOR)
    public void onPlayerHide(PlayerHideEvent event) {
        Optional<IChannel> channel = ChannelHandler.getInstance().getIntegrationChannel("events", Dependency.SUPERVANISH);
        ...
        builder.withFooterText(this.pair.getLeft());
        builder.withFooterIcon(this.pair.getRight());
